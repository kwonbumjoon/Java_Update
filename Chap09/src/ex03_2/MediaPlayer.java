...
/*문제: 고급 미디어 플레이어
다양한 유형의 미디어 파일을 재생하고, 각 파일에 대한 정보를 저장하며, 
플레이어의 상태를 관리하는 고급 미디어 플레이어를 구현합니다.
...
각 미디어 플레이어 클래스(MP3Player, WAVPlayer, FLACPlayer)에 파일 이름, 재생 시간, 
파일 크기 등의 정보를 저장하는 필드를 추가합니다.
loadFile(file) 메소드에서는 파일 정보를 해당 필드에 저장하고, 파일이 성공적으로 로드되었음을 
알리는 메시지를 출력합니다.
플레이어 상태 관리
